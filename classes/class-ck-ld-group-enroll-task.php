<?php 
/**
 * Handles WP User Learndash Group Enrollment and Course Completion
 *
 * @since      1.0.0
...
class CK_LD_Group_Enroll_Task {
    ...
    /**
     * @var array - Group Courses and Steps
     */
     ...
     * @param array $courses
     */
    public function __construct( int $user_id, int $group_id, array $courses = [], int $admin_id ) {
    ...
    public function run(){
        ...
        if( empty( $email ) ){
            $this->response['message'] = sanitize_text_field( sprintf(
    ...
    private function enroll_user_to_group() {
        ...
        if( ! empty($joined_groups) && in_array( $this->group_id, $joined_groups ) ){
    ...
    private function enroll_user_to_group_courses() {
        ...
        if( ! empty( $this->courses ) ){
            ...
            foreach ( $this->courses as $course_id => $course ){
                $in_course = in_array( $course_id, $enrolled );
                if( ! $in_course ){
    ...
    private function mark_courses_complete( array $courses ){
        ...
        return ' ' . sanitize_text_field( sprintf(
            /* translators: %d: number of courses completed, %d: total number of courses */
            __( '%d of %d courses completed', 'ck-ld-group-enroll'),
            $completed, 
            count($courses) 
        ) );
    }
