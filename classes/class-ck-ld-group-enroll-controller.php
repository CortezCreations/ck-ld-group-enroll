...
/**
 * CK LearnDash Group Enroll Controller Class
 */
class CK_LD_Group_Enroll_Controller {
	 ...
	 *
	 * @since 1.0.0
	 * @var array - $data - Settings Queue Data
	 */
	private $data = array();
	 ...
	 *
	 * @since 1.0.0
	 * @var mixed $error  null || WP_Error - Settings Queue Data
	 */
	private $error = null;
	 ...
	 *
	 * @since 1.0.0
	 * @var string ERROR_KEY  Error Key for WP_Error
	 */
	 ...
	 *
	 * @since 1.0.0
	 * @var array $user_ids - WP User IDs to be enrolled in the group
	 */
	 ...
	 *
	 * @since 1.0.0
	 * @var int $admin_id - WP User ID from the user that initiated the process.
	 */
	 ...
	 *
	 * @since 1.0.0
	 * @var int $admin_email - WP User email for the user who initiated the process.
	 */
	private $admin_email = '';
	...
	private function validate_task_data( array $data ) {
		...
		if ( ! $group ) {
			...
			$published = 'publish' === $group->post_status;
			if ( ! $published ) {
				$group = false;
	...
	private function set_task_data_to_process() {
		...
		$data['title'] = sanitize_text_field(
			sprintf(
				...
				__( 'Enrolling %1$d users into LearnDash group : %2$s', 'ck-ld-group-enroll' ),
				count( $this->user_ids ),
		...
		$this->data['messaging'][] = array(
			...
			'message' => sanitize_text_field(
				sprintf(
					...
					__( 'Initiated by admin %1$s @ %2$s', 'ck-ld-group-enroll' ),
					$this->admin_email,
	...
	private function set_completed_task_data() {
		...
		$data['title'] = sanitize_text_field(
			sprintf(
				...
				__( 'Enrolled %1$d users into LearnDash group : %2$s', 'ck-ld-group-enroll' ),
				$enrolled_count,
				$group_title
			)
		);
		...
		$data['messaging'][] = array(
			...
			'message' => sanitize_text_field(
				sprintf(
					...
					__( '%1$s @ : %2$s %3$s users processed', 'ck-ld-group-enroll' ),
					$verb,
					current_datetime()->format( 'Y-m-d H:i:s' ),
					$results_count
				)
	 ...
	 *
	 * @param int $user_id - User ID to run the task on.
